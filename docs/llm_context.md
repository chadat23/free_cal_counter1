...
## 1.4 Portion Edit: a place for a user to be able to edit how much of a particular food was eaten
...
- 1.4.7 Below the targets from 1.4.6 should be an output showing how many grams of said food are required in order to achieve the target amount of whatever the target is.
  - 1.4.7.1 If g is selected unit, and the target is Unit, and the Amount is 125, the output should be 125.
...
- 1.4.10 Presently, the Add button always says "Add", but ideally it would say "Update" if the user is updating a portion. This if this is a practical change then it should be implemented, but if it's too complicated then it shouldn't. 
...
## 1.6 Ingredient Edit: a place for a user to be able to edit how much of a ingredient is in a recipe.
...
- 1.6.2 Should have two rows of macros: the recipe's macro including the current proposed ingredient, and then the ingredient's macros. Both should update in real time.
- 1.6.3 If it's going to update an ingredient that's already been added to the recipe, the recipe's macros should be recipe's macros - old version of the ingredient + new version of the ingredient. When adding new ingredients, the recipe's macros should be the recipe's macros + the ingredient's macros.
...
- 1.6.10 The Update button should add the ingredient to the recipe if the user navigated to the Ingredient Edit screen by clicking to add an ingredient on the Edit Recipe Screen, and should update the existing ingredient on the recipe if the user navigated to the Ingredient Edit screen by clicking to edit an already present ingredient on the Edit Recipe Screen. The inconsistency in text on the Update button (how it always says Update even though it's sometimes adding (such as when the screen was reach via the Edit Recipe Add Ingredient button)) would ideally be fixed but is a very low priority so if doing so realistically almost any complication or technical risk it should be avoided for the foreseeable future.
...
- 1.7.6 Next should a button to add ingredients. this means a number of things must be configurable on the search screen:
  ...
  - 1.7.6.6 Clicking on the rest of the Search Result widget needs to bring up the Ingredient Edit screen, not the Portion Edit Screen.
